...
<script>
$(function(){
	$(document).on('click','#reservationID',function(){
     ...
     <table id="result" style="width: 95%;">
      <thead>
         <tr>
           ...
           <!-- <th style="width:10%;height:22px;text-align:center;font-size:15px;">예약결과번호</th> -->
...
<!--     <td>  </td> -->
    <td>
    	<form id="frm" method="post"> 
    		<a id="reservationID" >${result.m_uid }</a>
    		<input type ="hidden" id ="rs_id" value="${result.rs_id }"/>
    	</form>
    </td>
    <td> ${result.e_id } </td>
    <td> ${result.e_name } </td>
 ...
 <table style="width : 90%; height : 25px; align : center;" > 
	<tr>
		...
		<c:if test="${ showprev eq true }">
			<a href="${page}?nowpage=${prevnowpage}&pagecount=${pagecount}&pagegrpnum=${pagegrpnum-1}&r_result=${r_result}" style="text-decoration: none;">
				...
				&nbsp;&nbsp;
					...
					<c:choose>
						<c:when test="${pagenum eq nowpage }">
							<a href="${page}?nowpage=${pagenum}&pagecount=${pagecount}&pagegrpnum=${pagegrpnum}&r_result=${r_result}" style="text-decoration: none;">
							<b>${ pagenum }</b>
							</a>
						</c:when>					
						<c:otherwise>         			
		...
		<c:if test="${ shownext eq true }">
			<a href="${page}?nowpage=${nextnowpage}&pagecount=${pagecount}&pagegrpnum=${pagegrpnum+1}&r_result=${r_result}" style="text-decoration: none;">
