...
<div class="inwrap">
 ...
 <table style="width : 90%; height : 25px; align : center;" > 
	<tr>
		...
		<c:set var="final_date" 	value="${final_date }" />
		<c:set var="start_date" 	value="${start_date }" />

		<!-- --------------------------------------------------- -->
		<c:if test="${ showprev eq true }">
			<a href="${page}?nowpage=${prevnowpage}&pagecount=${pagecount}&pagegrpnum=${pagegrpnum-1}&s_date=${start_date}&f_date=${final_date}" style="text-decoration: none;">
				...
				&nbsp;&nbsp;
					...
					<c:choose>
						<c:when test="${pagenum eq nowpage }">
							<a href="${page}?nowpage=${pagenum}&pagecount=${pagecount}&pagegrpnum=${pagegrpnum}&s_date=${start_date}&f_date=${final_date}" style="text-decoration: none;">
							${ pagenum }
							</a>
						</c:when>					
						<c:otherwise>         			
		...
		<c:if test="${ shownext eq true }">
			<a href="${page}?nowpage=${nextnowpage}&pagecount=${pagecount}&pagegrpnum=${pagegrpnum+1}&s_date=${start_date}&f_date=${final_date}" style="text-decoration: none;">
