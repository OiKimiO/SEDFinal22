		...
		//$('.inwrap').html('!!!!');
			  ...
			  $.ajax({
				  ...
				  success  : function(partsList) {
							  ...
							  success  : function(data) {
								  ...
								  $.each(items, function(i, v){
									 ...
									 //$('#ul-estimate').css("padding-top":"30px"); 
					 				 ...
					 				 $('#ul-estimate').append('<li style="padding:1rem"><label for="checkbox-1">' + '2 Star' + '</label>' + '<input type="checkbox" name="' + 'checkbox-1' + '" id="' + 'checkbox-1' + '">');
					  ...
					  $('#ul-estimate').after('<ul><li style="font-weight:bold">부품 총액</li><li id="totalPrice" style="font-weight:bold">' + '원</li></ul>');
				  },
				  error   : function(xhr) {
					  alert(xhr.status + ':' + xhr.statusText);
				  }
...
<body>
	<div id="wrap">
		...
		<table id="tbl-doingAS">
			...
			<td colspan="2"  class="td_p_name"><input type="text" name="p_name" class="iptPname" value="SC16Z-1D5E"></td>
			</c:if>
			...
			<td colspan="2"><input type="text" name="s_num" value="SN00000013"><!-- <a href="#">구매 내역 조회</a> --></td>
			</c:if>
			<c:if test="${not empty productFreeInfo.s_num}">
			<td colspan="2">${ productFreeInfo.s_num }<input type="hidden" name="s_num" id="s_num" value="${ productFreeInfo.s_num }"></td>
			</c:if>
			</tr>
			...
			<tr>
			<td colspan="2">${productFreeInfo.freeornot } <input type="hidden" name="fin_gbn" value="0"> 
