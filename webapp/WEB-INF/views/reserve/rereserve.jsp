		...
		$.ajax({
			...
			,success  : function(data){				
				console.log(data);
					...
					$('#selectMonthDiv').html('');	
					$('#selectMonthDiv').append('<select name = "month"  id = "monthSelect" class ="timeClass" required></select> 월');	
					...
					$(function(){
						$(document).on('mouseout','select[name=month]',function(){
							$.ajax({
								...
								,success  : function(data){
									console.log(data);						
									$('#selectDayDiv').html('<select name = "day"  id = "monthSelect" class ="timeClass" required></select>일');
							...
							$(function(){
								...
								$.ajax({
									...
									,success  : function(data){
										console.log(data);					
										$('#selectClockDiv').html('');	
										$('#selectClockDiv').html('<select name = "clock"  id = "monthSelect" class ="timeClass" required></select>시');
										for (var i = 0; i < data.length; i++) {
											...
											$(function(){
												...
												$.ajax({
													...
													,success  : function(data){
														console.log(data);					
														$('#selectEnginDiv').html('');	
														$('#selectEnginDiv').html('<select name = "selectengin"  id="selectEngin" class ="timeClass" required></select>');
														for (var i = 0; i < data.length; i++) {
...
$(function(){
	$('input[name=rs_note_input]').blur(function(){
		console.log('오오키');
		$('#rs_note').attr('value',$('input[name=rs_note_input]').val());
...
<body>
	...
	<%@ include file="/WEB-INF/include/mheader.jspf" %>
  	<!-- section  -->
		<section>
			<div id = "hDiv"><h2>고객 재예약</h2></div>				
			<div class="inwrap">		
				<table id ="notice">
					<tr>
						<th>접수 번호</th>
						<td id ="redId">${r_id_max}</td>
						<th>처리 상태</th>
						<td>-</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td>
							<div id="timeId" >
								<div class = "leftDiv" >
									<input  name = "year"   id = "yearId"  type = "text" value="2019" readonly/>년 							
								</div>
								<div id ="selectMonthDiv" class = "leftDiv" >
									<select name = "month"  id = "monthSelect" class ="timeClass" required>
									</select>월
								</div>
								<div id = "selectDayDiv" class = "leftDiv" >
									<select name = "day"  class ="timeClass" required>
									</select>일
								</div>
								<div id = "selectClockDiv" class = "leftDiv" >
									<select name = "clock"  class ="timeClass" required>
									</select>시 
								</div>															
							</div> 
						</td>
					...
					<tr>
						...
						<div id ="selectEnginDiv" >
							<select name ="selectengin" id="selectEngin" style="width : 100px;" required>
					...
					<tr>
						<th>비고</th>
						<td colspan="3"><input type = "text" name="rs_note_input" value="${MReserve.rs_note}"></td>
					</tr>					
				</table>
					<form action="/Reserve/CreateReserve" id="frm" method = "get">
								...
								<input type="hidden" 	 name = "r_msg"       value ="${MReserve.r_msg}" /> 
								
								<input type="hidden" 	 name = "rs_note"  id="rs_note"   /> 
